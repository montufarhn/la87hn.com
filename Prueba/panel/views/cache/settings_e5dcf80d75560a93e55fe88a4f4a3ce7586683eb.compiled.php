...
<?php $this->startSection('content'); ?>
 ...
 <?php $__currentLoopData = $general; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $input): $loop = $this->incrementLoopIndices();  ?>

 <?php if( $input === 'override-share-image' ): ?>
 <div class="form-group">
 <label class="col-sm-2 control-label" for="override_share_image">Override Share Image</label>
 <div class="col-sm-10">
 <div class="input-group col-sm-6">
 <input type="text" name="override_share_image" id="override_share_image" class="form-control" value="<?php echo \htmlentities($_POST[ 'override_share_image' ] ?? ''??'', ENT_QUOTES, 'UTF-8', false); ?>">
 ...
 <?php if( !empty( $_POST[ 'override_share_image' ] ) && is_file( $_POST[ 'override_share_image' ] ) ): ?>
 <div class="logo-container"><br>
 <div class="override-image">
 <img src="<?php echo \htmlentities($_POST[ 'override_share_image' ]??'', ENT_QUOTES, 'UTF-8', false); ?>" width="auto" height="80">
 </div>
 ...
 foreach($__currentLoopData as $index => $lang): $loop = $this->incrementLoopIndices();  ?>
 <option data-html="<i class='fi ico-left fi-<?php echo \htmlentities($lang['flag']??'', ENT_QUOTES, 'UTF-8', false); ?>'></i> <?php echo \htmlentities($lang['name']??'', ENT_QUOTES, 'UTF-8', false); ?>" value="<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>"<?php echo \htmlentities(( ( $index === $_POST['default_lang' ] ) ? 'selected' : '' )??'', ENT_QUOTES, 'UTF-8', false); ?>>
 <?php echo \htmlentities($lang['name']??'', ENT_QUOTES, 'UTF-8', false); ?>
 ...
 <?php $__currentLoopData = $templatesList; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $index => $name): $loop = $this->incrementLoopIndices();  ?>
 <option value="<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>"<?php echo ( ( $index === $_POST[ 'template' ] ) ? 'selected' : '' ); ?>><?php echo \htmlentities($name??'', ENT_QUOTES, 'UTF-8', false); ?></option>
 <?php endforeach; $this->popLoop(); $loop = $this->getFirstLoop(); ?>
 </select>
 </div>
 <div class="col-sm-6">
 ...
 <div class="content form-content">
 <?php $__currentLoopData = $trackInfo; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $info): $loop = $this->incrementLoopIndices();  ?>

 <?php if($info === 'stats_refresh'): ?>
 ...
 <input type="number" name="stats_refresh" class="form-control" id="stats_refresh" placeholder="15" required="" value="<?php echo \htmlentities($_POST[ 'stats_refresh' ]??'', ENT_QUOTES, 'UTF-8', false); ?>">
 </div>
 </div>
 ...
 <?php echo $form->generateForm( $artworks ); ?>
 ...
 <input class="form-control" id="images_size" name="images_size" placeholder="280" type="number" value="<?php echo \htmlentities(( !empty( $_POST[ 'images_size' ] ) ? $_POST[ 'images_size' ] : '' )??'', ENT_QUOTES, 'UTF-8', false); ?>">
 </div>
 </div>
 ...
 <tbody>
 <?php $__currentLoopData = $artworkAPIs; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $key => $api): $loop = $this->incrementLoopIndices();  ?>
 <tr>
 <td style="width: 2rem">
 <div class="checkbox">
 <label>
 <input type="checkbox" name="<?php echo \htmlentities($key??'', ENT_QUOTES, 'UTF-8', false); ?>[state]" value="enabled" <?php echo ( ( isset($api['state']) && $api['state'] === 'enabled' ) ? 'checked' : '' ); ?>>
 ...
 <td style="width:30%">
 <input type="hidden" name="<?php echo \htmlentities($key??'', ENT_QUOTES, 'UTF-8', false); ?>[index]" value="$key">
 <b><?php echo \htmlentities($api['name']??'', ENT_QUOTES, 'UTF-8', false); ?></b>
 </td>
 <td class="table-input">
 <?php if(isset($api['field'])): ?>
 <div class="input-append">
 <input class="form-control" type="text"
 name="<?php echo \htmlentities($key??'', ENT_QUOTES, 'UTF-8', false); ?>[<?php echo \htmlentities($api['field']['name']??'', ENT_QUOTES, 'UTF-8', false); ?>]"
 placeholder="<?php echo \htmlentities($api['field']['placeholder']??'', ENT_QUOTES, 'UTF-8', false); ?>"
 value="<?php echo \htmlentities(( !empty( $api[ $api['field']['name'] ] ) ? $api[$api['field']['name'] ] : '' )??'', ENT_QUOTES, 'UTF-8', false); ?>">
 <div class="append css-hint left" data-title="<?php echo \htmlentities($api['field']['helpText'] ?? $api['field']['placeholder']??'', ENT_QUOTES, 'UTF-8', false); ?>">
 <?php if(!empty($api['field']['helpURL'])): ?>
 <a href="<?php echo \htmlentities($api['field']['helpURL']??'', ENT_QUOTES, 'UTF-8', false); ?>" target="_blank">
 ...
 <?php $__currentLoopData = $templates; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $index => $data): $loop = $this->incrementLoopIndices();  ?>
 <li>
 <a <?php echo ( ( $index === array_keys( $templates )[ 0 ] ) ? ' class="active"' : '' ); ?>data-bs-toggle="tab"
 data-bs-target="#<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>" href="#<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>"><?php echo \htmlentities($data[ 'name' ]??'', ENT_QUOTES, 'UTF-8', false); ?>
 ...
 <?php $__currentLoopData = $templates; $this->addLoop($__currentLoopData);$this->getFirstLoop();
 foreach($__currentLoopData as $index => $data): $loop = $this->incrementLoopIndices();  ?>
 <div role="tabpanel" id="<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>" class="tab-pane<?php echo \htmlentities(( ( $index === array_keys( $templates )[ 0 ] ) ? ' active' : '' )??'', ENT_QUOTES, 'UTF-8', false); ?>">
 ...
 <?php echo $panel->alert( 'Sorry, there are no advanced options available for this template.', 'info' ); ?>

 <?php else: ?>
 <input type="hidden" name="template" value="<?php echo \htmlentities($index??'', ENT_QUOTES, 'UTF-8', false); ?>">
 <?php /* Parse selected options */ ?>
 <?php $opts = $pawtunes->getAdvancedTemplateOptions( $index );  ?>
 <?php $__currentLoopData = $data[ 'extra' ]; $this->addLoop($__currentLoopData);$this->getFirstLoop();
