...
class Compiler {
    ...
    public function compile( $code, $path = null ) {
        ...
        if ( $sourceMap !== null ) {
            ...
            } else if ( $this->sourceMap === self::SOURCE_MAP_FILE ) {
    ...
    public function compileString( $source, $path = null ) {
        ...
        try {
            ...
            if ( $this->sourceMap ) {
                ...
                } else if ( $this->sourceMap !== self::SOURCE_MAP_NONE ) {
                    $sourceMapGenerator = new SourceMapGenerator( $this->sourceMapOptions );
                }
            }
    ...
    protected function compileChildrenNoReturn( $stms, OutputBlock $out, $selfParent = null, $traceName = '' ) {
        ...
        foreach ( $stms as $stm ) {
            ...
            } else if ( $selfParent && in_array( $stm[ 0 ], [ Type::T_INCLUDE, Type::T_EXTEND ] ) ) {
    ...
    protected function compileChild( $child, OutputBlock $out ) {
        ...
        } else if ( is_array( $child ) && isset( $child[ 1 ]->sourceLine ) && $child[ 1 ] instanceof Block ) {
            $this->sourceIndex = $child[ 1 ]->sourceIndex;
            $this->sourceLine = $child[ 1 ]->sourceLine;
            $this->sourceColumn = $child[ 1 ]->sourceColumn;
        } else if ( !empty( $out->sourceLine ) && !empty( $out->sourceName ) ) {
        ...
        switch ( $child[ 0 ] ) {
            ...
            case Type::T_ASSIGN:
                ...
                if ( in_array( $compiledName, [ 'font', 'grid-row', 'grid-column', 'border-radius' ] ) ) {
                    ...
                    } else if ( $shorthandValue[ 0 ] === Type::T_LIST ) {
    ...
    protected function reduce( $value, $inExp = false ) {
        ...
        switch ( $value[ 0 ] ) {
            case Type::T_EXPRESSION:
                ...
                } else if ( is_callable( [ $this, $fn = "op{$ucLType}{$ucRType}" ] ) ) {
                    $out = $this->$fn( $op, $left, $right, $shouldEval );
                } else if ( is_callable( [ $this, $fn = "op{$ucOpName}" ] ) ) {
                    $out = $this->$fn( $left, $right, $shouldEval );
                } else {
                    $out = null;
                }
    ...
    private function evaluateArguments( array $args, $reduce = true ) {
        ...
        foreach ( $args as $arg ) {
            if ( !empty( $arg[ 0 ] ) ) {
                ...
                } else if ( $val[ 0 ] !== Type::T_NULL ) { // values other than null are treated a single-element lists, while null is the empty list
                    $positionalArgs[] = $this->maybeReduce( $reduce, $val );
                }
            } else if ( $hasKeywordArgument ) {
        ...
        foreach ( [ array_reverse( $base ), array_reverse( $other ) ] as $single ) {
            ...
            foreach ( $single as $part ) {
                if ( preg_match( '/^[\[:]/', $part ) ) {
                    ...
                    $wasTag = false;
                } else if ( preg_match( '/^[^_-]/', $part ) && $rang === 1 ) {
                    $tag[] = $part;
                    $wasTag = true;
                } else if ( $wasTag ) {
    ...
    protected function isWith( $block, $with, $without ) {
        if ( isset( $block->type ) ) {
            ...
            if ( $block->type === Type::T_DIRECTIVE ) {
                ...
                } else if ( isset( $block->selectors ) && preg_match( ',@(\w+),ims', json_encode( $block->selectors ), $m ) ) {
                    return $this->testWithWithout( $m[ 1 ], $with, $without );
                } else {
                    return $this->testWithWithout( '???', $with, $without );
                }
            }
        } else if ( isset( $block->selectors ) ) {
...
will be an error in future versions of Sass.\n         on line $line of $fname";
    ...
    protected function libJoin( $args ) {
        ...
        switch ( $this->compileStringContent( $this->assertString( $sep, 'separator' ) ) ) {
            ...
            case 'auto':
                if ( $list1[ 1 ] !== '' || count( $list1[ 2 ] ) > 1 || !empty( $list1[ 'enclosing' ] ) && $list1[ 'enclosing' ] !== 'parent' ) {
                    $separator = $list1[ 1 ] ? : ' ';
                } else if ( $list2[ 1 ] !== '' || count( $list2[ 2 ] ) > 1 || !empty( $list2[ 'enclosing' ] ) && $list2[ 'enclosing' ] !== 'parent' ) {
                    $separator = $list2[ 1 ] ? : ' ';
                } else {
                    $separator = ' ';
                }
        ...
        if ( $bracketed === static::$true ) {
            $bracketed = true;
        } else if ( $bracketed === static::$false ) {
            $bracketed = false;
        } else if ( $bracketed === [ Type::T_KEYWORD, 'auto' ] ) {
            $bracketed = 'auto';
        } else if ( $bracketed === static::$null ) {
            $bracketed = false;
        } else {
