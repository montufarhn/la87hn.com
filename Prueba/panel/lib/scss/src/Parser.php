...
class Parser {
    ...
    protected function parseChunk() {
        ...
        if ( $this->matchChar( '}', false ) ) {
            ...
            } else if ( !$block instanceof ElseBlock && !$block instanceof ElseifBlock ) {
    ...
    private function registerPushedBlock( Block $b, $pos ) {
        ...
        if ( !$this->env ) {
            $b->children = [];
        } else if ( empty( $this->env->children ) ) {
        ...
        if ( $this->matchEscapeCharacter( $escapedout, true ) ) {
            if ( strlen( $escapedout ) === 1 ) {
                if ( !preg_match( ",\w,", $escapedout ) ) {
                    $out = '\\' . $escapedout;
                    return true;
                } else if ( !$keepEscapedNumber || !is_numeric( $escapedout ) ) {
                    $out = $escapedout;
                    return true;
                }
            }
    ...
    protected function string( &$out, $keepDelimWithInterpolation = false ) {
        ...
        while ( $this->matchString( $m, $delim ) ) {
            ...
            if ( $m[ 2 ] === '#{' ) {
                ...
                } else if ( $this->matchEscapeCharacter( $c ) ) {
                    $content[] = $c;
                } else {
